 ...
 */
public class BubbleSortByAuthorName implements Sort{
    ...
    public void sort(ArrayList<Book> list) {
        ...
        for (int i = 0; i < list.size(); ++i){
            for (int j = i; j < list.size(); ++j){
                ...
                char[] buffer_1 = new char[list.get(i).getAuthorName().length()];
                char[] buffer_2 = new char[list.get(j).getAuthorName().length()];
                list.get(i).getAuthorName().getChars(0, buffer_1.length, buffer_1, 0);
                list.get(j).getAuthorName().getChars(0, buffer_2.length, buffer_2, 0);
